...
class AuthController extends GetxController {
  ...
  var usernameorPhoneNumbercontroller = TextEditingController();

  @override
  void onInit() {
    super.onInit();
    isoflineMode.value = box.read("offline") ?? false;
    String? savedUserNameorphoneNumper = box.read("savedUserNameorphoneNumper");
    if (savedUserNameorphoneNumper != null) {
      usernameorPhoneNumbercontroller.text = savedUserNameorphoneNumper;
    }
  }
    ...
    try {
      ...
      if (result["success"] == true &&
          (result["Role"] == "مدرس" ||
              result["Role"] == "admin" ||
              result["Role"] == "مشرف")) {
                box.write("savedUserNameorphoneNumper", usernameOrPhoneNumber);
                print("savedUserNameorphoneNumper: $usernameOrPhoneNumber");
                
        Role.value = result["Role"];
        final Map<String, dynamic> responseData = result["userData"];

        final Map<String, dynamic> userData =
            responseData["user"] ?? responseData["student"] ?? {};
        ...
        errorMessage.value =
            result["message"] ??
            "بيانات الدخول غير صحيحة (Role: ${result["Role"]})";
        return false;
      }
    } catch (e) {
